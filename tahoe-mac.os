...
# determine the real user (the person who invoked sudo), with fallbacks
if [ -n "$SUDO_USER" ]; then
  REAL_USER="$SUDO_USER"
  REAL_UID="$SUDO_UID"
  REAL_GID="$SUDO_GID"
else
  # not run via sudo — use the current user
  REAL_USER="$(id -un)"
  REAL_UID="$(id -u)"
  REAL_GID="$(id -g)"
fi

# get that user's home directory in a portable way
# eval echo "~$REAL_USER" works on most Unix / macOS systems
REAL_HOME="$(eval echo "~$REAL_USER")"

# path to their Downloads
USER_DOWNLOADS="$REAL_HOME/Downloads"

echo "Invoked by user: $REAL_USER (uid:$REAL_UID gid:$REAL_GID)"
echo "Checking downloads at: $USER_DOWNLOADS"

# example: check for SnapCamMac.pkg in their Downloads
if [ -f "$USER_DOWNLOADS/SnapCamMac.pkg" ]; then
  echo "Found SnapCamMac.pkg in $USER_DOWNLOADS"
  # copy it to /tmp so installer can use it
  cp -f "$USER_DOWNLOADS/SnapCamMac.pkg" /tmp/SnapCamMac.pkg
else
  echo "SnapCamMac.pkg not found in $USER_DOWNLOADS"
  # optionally also check /root/Downloads (if run without sudo)
  if [ -f "/root/Downloads/SnapCamMac.pkg" ]; then
    echo "Found in /root/Downloads; copying"
    cp -f /root/Downloads/SnapCamMac.pkg /tmp/SnapCamMac.pkg
  else
    echo "Please put SnapCamMac.pkg in $USER_DOWNLOADS or /root/Downloads"
    exit 1
  fi
fi
